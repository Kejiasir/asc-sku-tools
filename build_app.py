...
macOS   -> dist/ASC SKU.app
Windows -> dist/ASC-SKU-windows.zip
        -> dist/ASC-SKU-Setup.exe  (if Inno Setup is installed)
...
def _package_windows(exe: Path) -> None:
    ...
    print(f"portable zip: {archive.with_suffix('.zip')}")
    ...
    if iscc and iss.is_file():
        ...
        print(f"installer: {ROOT / 'dist' / 'ASC-SKU-Setup.exe'}")
    else:
